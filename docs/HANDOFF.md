...
- The live release is **Version 66**, `BUILD_ID=984542a`.
...
- `/api/version` returns `{"version":66,"build":"984542a","deployedAt":"2026-08-26"}`.
- `/` returns 200 with `cache-control: no-store, must-revalidate` and held 200 across 60 consecutive requests.
- **Site visibility is "members" ("Only people I add", owner-flipped 2026-08-26)**: anonymous visitors get the sign-in gate on `/` and 401 from `/api/tree`, `/api/photos/*`, and `/api/ask`; only the accounts on the member list can view, and sign-ins do NOT auto-register while this mode is on. Admins flip it back on `/settings` → Members & access. Note the static legacy pages (`/legacy-family-tree*`, `/legacy-photos/*`) are served from assets and are NOT covered by this gate.
