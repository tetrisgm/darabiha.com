...
- The live release is **Version 51**, `BUILD_ID=3852748`, Worker deployment `e2aec8cf-9cf1-41c9-b2b0-5743c3fd721b`.
- The release spans commits `6543b81..3852748` (2026-08-26): the corrected legacy archive was imported into the live D1 tree, the canvas got a real family-tree layout, the root page was fitted into the Worker CPU budget, and the archive gained Family/Fan/List/Fill-in views, person search, branch folding, and universal couple adjacency.
...
- Production exposes its uncached identity at `/api/version` and as visible `Version 51` text at the lower-left edge.
...
- `/api/version` returns `{"version":51,"build":"3852748","deployedAt":"2026-08-26"}`.
...
- **Family** (`app/components/TreeViews.tsx FocusFamilyView`): one screen around a focal person — labeled Grandparents/Parents/Children rows, the focal couple with `⚭`, sibling chips, children grouped by marriage. Clicking a relative re-centers, a Back button walks the focal history (kept in the app shell), and a hint line explains the navigation. The initial focal person is Nasser Darabiha.
