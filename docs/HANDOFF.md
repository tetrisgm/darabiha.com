...
- The live release is **Version 64**, `BUILD_ID=b3b1d80`.
...
- Sign-in lives on `/settings` (Apple + Google); the header's Sign in pill points there. Editors today: `nasserdarabiha@gmail.com`, `parissima.d@gmail.com` (Google test users until the consent screen is published); admins: `ramine@ramine.net` with `leshokunin@gmail.com` linked.
...
- `/api/version` returns `{"version":64,"build":"b3b1d80","deployedAt":"2026-08-26"}`.
...
- **Google OAuth publishing is blocked by a Google console bug** (investigated exhaustively 2026-08-26): the Audience page for project `darabiha` shows "Your app's OAuth configuration is incomplete → visit the Branding page" and disables Publish app, while every Branding field (name, support email, home page, authorized domain `darabiha.com`, developer contact) is verifiably persisted — confirmed by DOM value reads and two full-record re-saves. Registering the three basic scopes, an Internal→External toggle, and selecting "In production" directly in the Make-external dialog were all refused; the server enforces the incompleteness flag. A Google Developer forum thread (discuss.google.dev/t/392229) documents this exact symptom with no working fix. Do NOT re-try the UI dance; the options are: wait (such flags have cleared on their own), escalate to Google, or rebuild the auth config in a fresh project and rotate the client (owner decision). Until then Google sign-in runs in Testing mode: the three registered test users can sign in, with re-consent every 7 days.
