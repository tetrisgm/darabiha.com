...
const QUESTIONS = [
  {
    ...
    actionSummary: "If confirmed, Fatemeh Massoudi is linked as a daughter of Haj Mirza Agha Masoudi and her record notes she was Robabeh's sister.",
    actions: [
  ...
  },
  {
    ...
    actionSummary: "If confirmed, Ashraf's record notes that her father was the partnership's bookkeeper of the marriage story.",
    actions: [
...
for (const entry of QUESTIONS) {
  statements.push(`INSERT OR IGNORE INTO open_questions (id, question, evidence, action_summary, proposal_json, status, created_at) VALUES (${q(entry.id)}, ${q(entry.question)}, ${q(entry.evidence)}, ${q(entry.actionSummary)}, ${q(entry.actions ? JSON.stringify({ actions: entry.actions }) : null)}, 'open', ${q(now)});`);
  report.push(entry.question);
}
