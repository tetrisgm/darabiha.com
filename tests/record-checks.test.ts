...
describe("record checks", () => {
  ...
  it("credits a story's claim to its subject only", () => {
    const checks = runRecordChecks(tree(
      [person("m", "Mohammad Zehtab Darabi", { birthDate: "1856", deathDate: "1939" }), person("h", "Hossein Zehtab Darabi", { birthDate: "1882", deathDate: "1937" })],
      [],
      [{ id: "s", title: "Mohammad Zehtab (Darabi), fourth generation", body: "He was over a hundred years old. Hossein ran the company.", date: null, place: null, personIds: ["m", "h"] }],
    ));
  ...
  it("asks about same-name records only when the archive cannot tell them apart", () => {
    const ambiguous = runRecordChecks(tree(
      [person("a", "Abbas Darabi"), person("b", "Abbas Darabi"), person("p", "A Parent")],
      [parent("p", "a")],
    ));
    expect(ambiguous.map((check) => check.kind)).toEqual(["duplicate"]);

    const distinguished = runRecordChecks(tree(
      [person("a", "Abbas Darabi"), person("b", "Abbas Darabi"), person("p1", "One"), person("p2", "Two")],
