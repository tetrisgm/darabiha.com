...
export function FamilyTreeCanvas({ tree, onSelect }: { tree: FamilyTree; onSelect: (person: Person) => void }) {
  ...
  return <div className="family-canvas"><Canvas camera={{ position: [0, 0, 13], fov: 42 }} dpr={[1, 2]} gl={{ antialias: true }}>
    ...
    <OrbitControls enablePan enableRotate={false} minDistance={5} maxDistance={24} zoomSpeed={0.8} panSpeed={0.8} /></Canvas><div className="safari-tree-fallback" aria-hidden="true"><svg className="safari-tree-lines" viewBox="0 0 100 100" preserveAspectRatio="none">{tree.relationships.map((link) => { const from = positions.get(link.fromPersonId); const to = positions.get(link.toPersonId); if (!from || !to) return null; const x1 = 50 + from[0] * 8; const y1 = 50 - from[1] * 9; const x2 = 50 + to[0] * 8; const y2 = 50 - to[1] * 9; return link.type === "spouse" ? <line key={link.id} x1={x1} y1={y1} x2={x2} y2={y2} /> : <path key={link.id} d={`M ${x1} ${y1} V ${(y1 + y2) / 2} H ${x2} V ${y2}`} />; })}</svg>{tree.people.map((person) => { const point = positions.get(person.id) ?? [0, 0, 0]; return <button className="safari-tree-card" style={{ left: `${50 + point[0] * 8}%`, top: `${50 - point[1] * 9}%` }} onClick={() => onSelect(person)} key={person.id}><strong>{person.displayName}</strong><span>{person.birthDate ? person.birthDate.slice(0, 4) : "Year unknown"}</span></button>; })}</div></div>;
}
