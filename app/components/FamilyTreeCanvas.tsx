...
export function FamilyTreeCanvas({ tree, onSelect }: { tree: FamilyTree; onSelect: (person: Person) => void }) {
  ...
  return <div className="family-canvas"><Canvas camera={{ position: [0, 0, 13], fov: 42 }} dpr={[1, 2]} gl={{ antialias: true }}>
    ...
    <OrbitControls enablePan enableRotate={false} minDistance={5} maxDistance={24} zoomSpeed={0.8} panSpeed={0.8} /></Canvas><div className="safari-tree-fallback" aria-hidden="true"><svg className="safari-tree-lines" viewBox="0 0 100 100" preserveAspectRatio="none">{tree.relationships.map((link) => { const from = tree.people.findIndex((person) => person.id === link.fromPersonId); const to = tree.people.findIndex((person) => person.id === link.toPersonId); if (from < 0 || to < 0) return null; const x1 = ((from + .5) / tree.people.length) * 100; const x2 = ((to + .5) / tree.people.length) * 100; return <line key={link.id} x1={x1} y1="50" x2={x2} y2="50" />; })}</svg>{tree.people.map((person) => <div className="safari-tree-card" key={person.id}><strong>{person.displayName}</strong><span>{person.birthDate ? person.birthDate.slice(0, 4) : "Year unknown"}</span></div>)}</div></div>;
}
