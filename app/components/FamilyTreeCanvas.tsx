...
export function FamilyTreeCanvas({ tree, onSelect }: { tree: FamilyTree; onSelect: (person: Person) => void }) {
  ...
  return <div className="family-canvas"><Canvas camera={{ position: [0, 0, 13], fov: 42 }} dpr={[1, 2]} gl={{ antialias: true }}>
    ...
    <OrbitControls enablePan enableRotate={false} minDistance={5} maxDistance={24} zoomSpeed={0.8} panSpeed={0.8} /></Canvas><div className="safari-tree-fallback" aria-hidden="true">{tree.people.map((person) => <div className="safari-tree-card" key={person.id}><strong>{person.displayName}</strong><span>{person.birthDate ? person.birthDate.slice(0, 4) : "Year unknown"}</span></div>)}</div></div>;
}
