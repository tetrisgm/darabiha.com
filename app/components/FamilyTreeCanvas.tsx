...
export function FamilyTreeCanvas({ tree, onSelect }: { tree: FamilyTree; onSelect: (person: Person) => void }) {
  ...
  const setHoverCursor = (cursor: "grab" | "pointer" | "auto") => {
  ...
  const end = () => { if (gesture.current?.moved) lastDragAt.current = Date.now(); gesture.current = null; };
