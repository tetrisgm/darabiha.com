...
function contradictory(existing: Person, incoming: AddPersonProposal["person"]) {
...
function mergePerson(existing: Person, incoming: AddPersonProposal["person"]): Omit<Person, "id"> {
  return {
...
export function reconcileProposals(tree: FamilyTree, proposals: ChangeProposal[]) {
  ...
  for (const proposal of proposals) {
    ...
    const matches = tree.people.filter((person) => normalizedName(person.displayName) === normalizedName(proposal.person.displayName));
    if (!matches.length) { accepted.push(proposal); continue; }
